...
const meditationSessions = daySessions.filter(s => s.type === 'meditation');
    const med = meditationSessions[0] || null;
    // sessionData = flat {hr, hrv} array — what the dashboard stats useMemo iterates directly
    const sessionData = med ? parseSampleData(med.heart_rate, med.heart_rate_variability) : [];
    // Store session summary separately for duration/settle/mood display
    const medStats = computeStats(sessionData);
    const medDuration = med ? Math.round((new Date(med.end_datetime) - new Date(med.start_datetime)) / 60000) : 0;
    let settleMin = 0;
    if (sessionData.length && medStats.heartRateAvg > 0) {
      const threshold = medStats.heartRateAvg * 0.95;
      const settlePoint = sessionData.find(p => p.hr <= threshold);
