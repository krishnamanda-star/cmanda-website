    ...
    try{
      ...
      if(accessToken){
        ...
        if(existingBin){
          ...
          try{
            var fr=await fetch("/api/strava",{method:"POST",headers:{"Content-Type":"application/json"},
              body:JSON.stringify({action:"load",athlete_id:athlete.id})});
            var fd=await fr.json();
            if(fd.bin_id&&fd.data){
              binId=fd.bin_id;
          ...
          }catch(e){ console.warn("Cloud lookup failed, starting fresh:",e); }
        }
        saveLocal();
    ...
    if(binId){
      ...
      try{
        var fr=await fetch("/api/strava",{method:"POST",headers:{"Content-Type":"application/json"},
          body:JSON.stringify({action:"load",athlete_id:athlete.id})});
        var fd=await fr.json();
        if(fd.bin_id&&fd.data){ binId=fd.bin_id; localStorage.setItem("bikemaint_bin_"+athlete.id,binId); await loadFromCloud(athlete.id,binId); }
      }catch(e){}
    }
